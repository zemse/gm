...
impl TokenCreatePage{
    pub fn new(token_index: usize, network_index: usize, network: Network) -> crate::Result<Self>
    {
        ...
        Ok(Self {
            ...
            token_index,
            remove_popup: ConfirmPopup::new("Remove Token", "Are you sure you want to remove this token?".to_string(), "Remove", "Cancel"),
            token,
            network,
            network_index,
    ...
    pub fn token(form: &Form<FormItem> ) -> Token {
        Token {
            ...
            decimals:  form.get_text(FormItem::Decimals).parse().unwrap_or_default(),
            contract_address:  form.get_text(FormItem::ContractAddress).clone().parse::<Address>().unwrap_or_default(),
        }
    }
}
impl Component for TokenCreatePage {
    fn handle_event(&mut self, event: &Event, area: Rect, transmitter: &Sender<Event>, shutdown_signal: &Arc<AtomicBool>, shared_state: &SharedState) -> crate::Result<HandleResult> {
        ...
        if self.remove_popup.is_open() {
            let r = self.remove_popup.handle_event(event, area, || {
                ...
                handle_result.page_inserts.push(Page::Token(
                    TokenPage::new(self.network_index, self.network.clone())?
                ));
