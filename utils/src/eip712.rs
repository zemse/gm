...
pub fn eip712_to_dyn(
    ...
    let types = td
        .get("types")
        .ok_or(crate::Error::EIP712("missing types"))?;
    let domain = td
        .get("domain")
        .ok_or(crate::Error::EIP712("missing domain"))?;
    let primary = td
        .get("primaryType")
        .and_then(|v| v.as_str())
        .ok_or(crate::Error::EIP712("missing primaryType"))?;
    let message = td
        .get("message")
        .ok_or(crate::Error::EIP712("missing message"))?;
...
impl TypeEnv {
    fn build(types: &Value) -> crate::Result<Self> {
        let obj = types
            .as_object()
            .ok_or(crate::Error::EIP712("types must be an object"))?;
        ...
        for (name, fields_v) in obj {
            let fields = fields_v
                .as_array()
                .ok_or(crate::Error::EIP712String(format!(
    ...
    fn build_struct_and_value(
        ...
        for (fname, fty) in spec.field_names.iter().zip(spec.field_types.iter()) {
            let field_json = obj.get(fname).ok_or(crate::Error::EIP712String(format!(
    ...
    fn build_value_from_type(&self, ty: &DynSolType, v: &Value) -> crate::Result<DynSolValue> {
        Ok(match ty {
            ...
            DynSolType::String => DynSolValue::String(
                v.as_str()
                    .ok_or(crate::Error::EIP712("expected string"))?
                    .to_string(),
            ),
            ...
            DynSolType::Array(inner) => {
                let arr = v.as_array().ok_or(crate::Error::EIP712("expected array"))?;
                DynSolValue::Array(
                    arr.iter()
            ...
            DynSolType::FixedArray(inner, n) => {
                let arr = v.as_array().ok_or(crate::Error::EIP712("expected array"))?;
                if arr.len() != *n {
                    return Err(crate::Error::EIP712String(format!(
            ...
            DynSolType::Tuple(_field_tys) => {
                let obj = v
                    .as_object()
                    .ok_or(crate::Error::EIP712("expected object for tuple/struct"))?;
                ...
                let (_cand_ty, cand_spec) = found.ok_or(crate::Error::EIP712(
                    ...
                    .zip(cand_spec.field_types.iter())
                {
                    let field_json = obj
                        .get(fname)
                        .ok_or(crate::Error::EIP712String(format!("missing field {fname}")))?;
...
fn parse_address(v: &Value) -> crate::Result<Address> {
    let s = v
        .as_str()
        .ok_or(crate::Error::EIP712("address must be string"))?;
    s.parse::<Address>()
        .map_err(|e| crate::Error::EIP712String(format!("invalid address: {e}")))
}
...
fn parse_bool(v: &Value) -> crate::Result<bool> {
    ...
    Err(crate::Error::EIP712("invalid bool"))
}

fn parse_bytes(v: &Value) -> crate::Result<Vec<u8>> {
    let s = v
        .as_str()
        .ok_or(crate::Error::EIP712("bytes must be hex string"))?;
    let s = s.strip_prefix("0x").unwrap_or(s);
    hex::decode(s).map_err(|e| crate::Error::EIP712String(format!("hex decode error: {e}")))
}
...
fn parse_fixed_bytes(v: &Value, n: usize) -> crate::Result<[u8; 32]> {
    ...
    let s = v
        .as_str()
        .ok_or(crate::Error::EIP712("bytesN must be hex string"))?;
    let s = s.strip_prefix("0x").unwrap_or(s);
    let raw =
        hex::decode(s).map_err(|e| crate::Error::EIP712String(format!("hex decode error: {e}")))?;
    if raw.len() != n {
...
fn parse_u256(v: &Value) -> crate::Result<U256> {
    if let Some(s) = v.as_str() {
        ...
        let bytes = hex::decode(s2)
            .map_err(|e| crate::Error::EIP712String(format!("hex decode error: {e}")))?;
        return Ok(U256::from_be_slice(&bytes));
    }
    if let Some(n) = v.as_u64() {
        return Ok(U256::from(n));
    }
    Err(crate::Error::EIP712("invalid uint"))
}
...
fn parse_i256(v: &serde_json::Value) -> crate::Result<I256> {
    if let Some(s) = v.as_str() {
        if let Some(hex) = s.strip_prefix("0x") {
            ...
            let mag = U256::from_str_radix(digits, 10)
                .map_err(|e| crate::Error::EIP712String(format!("invalid decimal int: {e}")))?;
    ...
    Err(crate::Error::EIP712("invalid int"))
}
...
fn resolve_type(
    ...
    Err(crate::Error::EIP712String(format!("unknown type: {ty}")))
}
...
fn parse_uint_bits(s: &str) -> crate::Result<Option<usize>> {
    ...
    if let Some(rest) = s.strip_prefix("uint") {
        ...
        let bits = rest
            .parse::<usize>()
            .map_err(|_| crate::Error::EIP712String(format!("invalid uint bits: {s}")))?;
...
fn parse_int_bits(s: &str) -> crate::Result<Option<usize>> {
    ...
    if let Some(rest) = s.strip_prefix("int") {
        ...
        let bits = rest
            .parse::<usize>()
            .map_err(|_| crate::Error::EIP712String(format!("invalid int bits: {s}")))?;
...
fn parse_bytes_n(s: &str) -> crate::Result<Option<usize>> {
    if let Some(rest) = s.strip_prefix("bytes") {
        ...
        let n = rest
            .parse::<usize>()
            .map_err(|_| crate::Error::EIP712String(format!("invalid bytesN: {s}")))?;
        if n == 0 || n > 32 {
            return Err(crate::Error::EIP712String(format!(
...
fn parse_array_type(s: &str) -> crate::Result<Option<(&str, Option<usize>)>> {
    if let Some(i) = s.find('[') {
        ...
        let n = inside
            .parse::<usize>()
            .map_err(|_| crate::Error::EIP712String(format!("invalid fixed array size in: {s}")))?;
